...
// ─────────────────────────────────────────────────────────────
// [SEC-05-TEMP] seedMandatePilot — ONE-SHOT HTTP admin seed
// INV-FORJA9-01 · 2026-06-24
// REMOVE from index.js after first successful call + redeploy.
// Protected by SEED_SECRET query param — sufficient for a one-shot op.
// URL: https://us-central1-aip-v1-3f57c.cloudfunctions.net/seedMandatePilot?secret=BREEDERHUB2026
// ─────────────────────────────────────────────────────────────
...
exports.seedMandatePilot = onRequest({ cors: false }, async (req, res) => {
    ...
    if (existing.exists) {
        return res.json({ ok: true, skipped: true, id: MANDATE_ID });
    }

    const { Timestamp: TS, FieldValue: FV } = require('firebase-admin/firestore');

    await ref.set({
        mandateId:      MANDATE_ID,
        createdAt:      TS.fromDate(new Date('2026-01-06T00:00:00Z')),
